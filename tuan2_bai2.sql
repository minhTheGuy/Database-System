...
use Quanlybanhang
-- cau 1:Sử dụng câu lệnh CREATE TABLE để tạo các bảng trong cơ sở dữ liệu như sơ đồ ở 
-- trên (bạn tự lựa chọn kiểu dữ liệu cho phù hợp).
...
(
	...
	TenHang varchar(10),
	MaCongTy varchar(10),
	MaLoaiHang varchar(8),
	Soluong int,
	DonViTinh varchar(3),
	GiaHang int
)
alter table MatHang add constraint FK_MatHang_MaHang Foreign key(MaCongTy) references NhaCungCap(MaCongTy)
alter table MatHang add constraint FK_MatHang_MaLoaiHang Foreign key(MaLoaiHang) references LoaiHang(MaLoaiHang)
...
alter table ChiTietDatHang add constraint FK_ChiTietDatHang_MaHang Foreign key(MaHang) references MatHang(MaHang)
alter table ChiTietDatHang add constraint FK_ChiTietDatHang_SoHoaDon Foreign key(SoHoaDon) references DonDatHang(SoHoaDon)
...
(
	SoHoaDon varchar(8) primary key,
	MaKhachHang varchar(10),
	MaNhanVien varchar(6),
	NgayDatHang smalldatetime,
	NgayGiaoHang smalldatetime,
	NgayChuyenHang smalldatetime,
	NoiGiaoHang varchar(30)
)
alter table DonDatHang add constraint FK_DonDatHang_MaNhanVien Foreign key(MaNhanVien) references NhanVien(MaNhanVien)
...
alter table DonDatHang add constraint FK_KhachHang_MaKhachHang Foreign key(MaKhachHang) references KhachHang(MaKhachHang)
-- cau 2:Bổ sung ràng buộc thiết lập giá trị mặc định bằng 1 cho cột SOLUONG và bằng 0
-- cho cột MUCGIAMGIA trong bảng CHITIETDATHANG.

alter table ChiTietDatHang add constraint df_SoLuong Default 1 for SoLuong, constraint df_MucGiamGia default 0 for MucGiamGia

-- cau 3: Bổ sung cho bảng DONDATHANG ràng buộc kiểm tra ngày giao hàng và ngày
-- chuyển hàng phải sau hoặc bằng với ngày đặt hàng.

alter table DonDatHang add constraint check_NgayGiao_NgayChuyen check(NgayDatHang > NgayGiaoHang)
-- cau 4: Bổ sung ràng buộc cho bảng NHANVIEN để đảm bảo rằng một nhân viên chỉ có thể
-- làm việc trong công ty khi đủ18 tuổi và không quá 60 tuổi.
alter table NhanVien add constraint check_NgaySinh check(year(getdate()) - year(NgaySinh) >= 18 and
														year(getdate()) - year(NgaySinh) <= 60)
-- cau 5: